...
namespace {
std::map<std::string, int> model_name_map2 {
  std::make_pair("deeplab_v3_plus_mobilenet_v2", 0),
};
}  // namespace

MaceStatus CreateMaceEngineFromCode2(
  ...
  switch (model_name_map2[model_name]) {
    case 0:
      net_def = mace::deeplab_v3_plus_mobilenet_v2::CreateNet();
      engine->reset(new mace::MaceEngine(device_type));
      model_data = mace::deeplab_v3_plus_mobilenet_v2::LoadModelData();
